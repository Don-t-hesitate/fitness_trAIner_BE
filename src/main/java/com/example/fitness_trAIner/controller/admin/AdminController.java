...
public class AdminController {
    ...
    @ApiResponse(responseCode = "400", description = "에러 발생", content = @Content(schema = @Schema(implementation = GlobalExceptionResponse.class)))
    public final GlobalResponse<List<AdminServiceUserFoodPreferencesResponse>> getAllUserFoodPreferences() {
        return GlobalResponse.<List<AdminServiceUserFoodPreferencesResponse>>builder()
                .message("사용자 식단 선호도 조회")
                .result(adminService.getAllUserFoodPreferences())
                .build();
    }

    @PutMapping("/food/preferences/{id}")
    @Operation(summary = "사용자 식단 선호도 수정", description = "사용자 식단 선호도 수정")
    @ApiResponse(responseCode = "200", description = "성공", useReturnTypeSchema = true)
    @ApiResponse(responseCode = "400", description = "에러 발생", content = @Content(schema = @Schema(implementation = GlobalExceptionResponse.class)))
    public final GlobalResponse<String> updateUserFoodPreferences(@RequestBody AdminUserPrefUpdateRequestBody requestBody) {
        return GlobalResponse.<String>builder()
    ...
    public ResponseEntity<Resource> getExcelFile() throws IOException {
        byte[] bytes = adminService.getExcelFileBytes();
